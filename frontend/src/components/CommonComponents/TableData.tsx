...
const DataTable: React.FC<DataTableProps> = ({ columns, data, actions }) => {
    ...
    const renderCellContent = (item: any, column: string) => {
        ...
        const value = item[columnMap[column]]

        if (column === "verificationStatus") {
            return (
                <span
                    className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${value === "approved"
                ...
                >
                    <div
                        className={`w-1.5 h-1.5 rounded-full mr-2 ${value === "approved" ? "bg-emerald-500" : value === "rejected" ? "bg-red-500" : "bg-amber-500"
                            }`}
                    />
                    {value}
                </span>
